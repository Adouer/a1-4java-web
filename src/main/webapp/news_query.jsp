...
<%@page import="com.adouer.a14javaweb.entity.News"%>
...
<title>Insert title here</title>
 ...
 <script>
 	...
 	//检查用户积分是否够用
	function checkScore(userscore, downloadscore){
		
		if(userscore < downloadscore){

			alert("您只剩下" + userscore + "分，该附件需要" + downloadscore + "分，无法下载");

			return false;

		}else{
			
			if(!confirm("下载该附件将扣除您" + downloadscore + "分，确定下载吗")){
				return false;
			}
		}
		 	...
		 	<td align="center">
				...
				<td align="center">
					...
					<c:if test="${news.attachment!=''}">
						...
						<a href="NewsDownloadServlet?attachment=${news.attachment}&truename=${news.truename}&downloadscore=${news.downloadscore}&newsid=${news.newsid}" onclick="return checkScore(${sessionScope.user.score}, ${news.downloadscore})">
						
							<%-- <img src="image/icon/${fn:substringAfter(news.attachment, '.') }.gif"> --%>
							<img src="image/icon/${myfn:getIconFilename(news.attachment, pageContext.servletContext) }"/>

						</a>
						
						${myfn:getFilesize(news.attachment, pageContext.servletContext) }
